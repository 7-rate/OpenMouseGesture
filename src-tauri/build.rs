...
fn generate_license_file() {
    ...
    let mut html = fs::read_to_string(temp_file)
        .expect("Failed to read temporary license file");
    
    html = html.replace("<a href=", "<a target=\"_blank\" href=");
    
    let mut file = fs::File::create("license.html")
        .expect("Failed to create license.html");
    file.write_all(html.as_bytes())
        .expect("Failed to write license.html");
    
    fs::remove_file(temp_file)
        .expect("Failed to remove temporary license file");
