...
pub fn execute_action_with_window(action: &Action, target_window: Option<HWND>, activate_window: bool) -> std::result::Result<(), String> {
        ...
        unsafe {
            if hwnd != HWND::default() && IsWindow(Some(hwnd)).as_bool() {
                ...
                let target_hwnd = if is_child {
                    ...
                    eprintln!("[DEBUG] Detected child window, using root: HWND({:?}) -> HWND({:?})", hwnd, root);
                    root
                } else {
                    hwnd
                };
...
fn execute_keystroke_chord(keys: &[&str]) -> std::result::Result<(), String> {
    ...
    for k in keys {
        let vk = map_key_name_to_vkey(k)
            .ok_or_else(|| format!("Unknown key name: {}", k))?;
    ...
    eprintln!("[DEBUG] Total modifiers: {}, main_key: {:?}", modifiers.len(), main_key.map(|v| format!("0x{:02X}", v.0)));
    ...
    unsafe {
        if !modifiers.is_empty() {
            ...
            eprintln!("[DEBUG] SendInput(mod-down): sent {}/{} inputs", sent, inputs.len());
            if sent != inputs.len() as u32 {
                return Err(format!("SendInput(mod-down) failed: sent {}/{}", sent, inputs.len()));
            }
        ...
        if let Some(vk) = main_key {
            ...
            eprintln!("[DEBUG] SendInput(main-down): sent {}/{} inputs, VK=0x{:02X}, use_scancode={}", sent_down, inputs.len(), vk.0, !is_alnum_vk(vk) && !is_media_key(vk));
            if sent_down != inputs.len() as u32 {
                return Err(format!("SendInput(main-down) failed: sent {}/{}", sent_down, inputs.len()));
            }
            ...
            eprintln!("[DEBUG] SendInput(main-up): sent {}/{} inputs", sent_up, inputs.len());
            if sent_up != inputs.len() as u32 {
                return Err(format!("SendInput(main-up) failed: sent {}/{}", sent_up, inputs.len()));
            }
        ...
        if !modifiers.is_empty() {
            ...
            let sent = SendInput(&inputs, std::mem::size_of::<INPUT>() as i32);
            eprintln!("[DEBUG] SendInput(mod-up): sent {}/{} inputs", sent, inputs.len());
            if sent != inputs.len() as u32 {
                return Err(format!("SendInput(mod-up) failed: sent {}/{}", sent, inputs.len()));
            }
...
fn execute_command(action: &Action) -> std::result::Result<(), String> {
    let command = action.command.as_ref().ok_or("Execute action requires command field")?;
...
fn execute_browser(action: &Action) -> std::result::Result<(), String> {
    let url = action.url.as_ref().ok_or("Browser action requires url field")?;
...
fn execute_window_operation(action: &Action, preferred_hwnd: Option<HWND>) -> std::result::Result<(), String> {
    ...
    unsafe {
        ...
        match operation.as_str() {
            "minimize" => {
                ...
                eprintln!("  APPWINDOW: {}, TOOLWINDOW: {}", has_appwindow, has_toolwindow);
                eprintln!("  VISIBLE: {}, POPUP: {}, CHILD: {}", is_visible, is_popup, is_child);
                ...
                if let Err(e) = PostMessageW(Some(target_hwnd), WM_SYSCOMMAND, WPARAM(SC_MINIMIZE as usize), LPARAM(0)) {
                ...
                eprintln!("[DEBUG] After minimize - IsIconic: {}, IsVisible: {}", is_iconic, is_still_visible);
            }
            "maximize" => {
                let result = ShowWindow(target_hwnd, SW_SHOWMAXIMIZED);
                if !result.as_bool() {
                    eprintln!("[DEBUG] ShowWindow returned false, but operation may have succeeded");
                }
            }
            "close" => {
